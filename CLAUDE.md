...
A student receives a Slack message from one tracked sender → gets a translated, captioned vertical reel in Telegram →
replies in their own language → approves an English draft → it posts into the Slack thread.
...
| `supabase/migrations` | Postgres schema. `0001_init.sql` + `0002_context.sql` are applied on the hosted project | – |
...
- Ownership: Dev A = pipeline/engine/worker/shared/migrations; Dev B = Slack/Telegram/reply/API/web.
