...
const SOURCE_LABELS: Record<ReelArtifact["source"], string> = {
  slack: "Slack",
  gmail: "Gmail",
  outlook: "Outlook",
  instagram: "Instagram",
...
/** The inbound channel the message came from (Composio toolkit or Slack), shown in every card header. */
function sourceLabel(source: ReelArtifact["source"]): string {
  return SOURCE_LABELS[source] ?? "Inbox";
}
