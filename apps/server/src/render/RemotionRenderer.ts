...
export async function prepareRenderer(): Promise<string> {
  if (!bundlePromise) {
    bundlePromise = (async () => {
      ...
      return bundle({ entryPoint: path.join(repoRoot, "packages/reel/src/index.ts"), rootDir: path.join(repoRoot, "packages/reel"), publicDir, outDir: path.join(dataDir, "bundle"), enableCaching: true });
...
export class RemotionRenderer implements ReelRenderer {
  ...
  async render(interp: TimedInterpretation, _prefs: UserPreferences): Promise<ReelArtifact> {
    ...
    const composition = await selectComposition({ serveUrl, id: "Reel", inputProps, timeoutInMilliseconds: config.RENDER_TIMEOUT_MS });
    ...
    for (const crf of [26, 30]) {
      ...
      try {
        await renderMedia({ composition, serveUrl, inputProps, codec: "h264", crf, audioCodec: "aac", concurrency: 2, outputLocation, timeoutInMilliseconds: config.RENDER_TIMEOUT_MS, cancelSignal, imageFormat: "jpeg", overwrite: true });
