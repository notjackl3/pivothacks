...
export async function persistMessage(userId: string, message: NormalizedMessage, isMock = message.provider === "mock"): Promise<{ message: MessageRow; inserted: boolean }> {
  ...
  const expectedConnectionProvider = message.provider === "slack" && connection?.provider === "composio_slack" ? "slack" : connection?.provider;
  if (!connection || expectedConnectionProvider !== message.provider) throw Object.assign(new Error("Connection not available."), { statusCode: 404, code: "NOT_FOUND" });
  const row = {
    ...
    original_text: message.text, received_at: message.receivedAt, is_mock: isMock, source_provider: message.provider,
  };
...
export function normalizeStoredMessage(row: MessageRow): NormalizedMessage {
  return { provider: row.source_provider ?? (row.is_mock ? "mock" : "slack"), connectionId: row.connection_id, externalMessageId: row.external_message_id, externalChannelId: row.external_channel_id, externalThreadId: row.external_thread_id, senderExternalId: row.sender_external_id, senderDisplayName: row.sender_display_name, text: row.original_text, receivedAt: row.received_at };
}
